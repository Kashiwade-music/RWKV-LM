    ...
    def __init__(self, args):
        ...
        with torch.no_grad():
            ...
            if "pos_emb_x" in keys:
                w["pos_emb"] = (w["pos_emb_x"] + w["pos_emb_y"]).reshape(args.ctx_len + 1, -1)[
                    :-1, :
                ]
            keys = list(w.keys())
            ...
            for x in keys:
                ...
                if ".time_decay" in x:
                    ...
                    if self.FLOAT_MODE == "fp32":
                        ...
                        w[x] = w[x].half()
                    elif self.FLOAT_MODE == "int8":
                        w[x] = w[x].to(torch.int8)
                ...
                if ("blocks." not in x) or ("blocks.0." in x):
                    if print_need_newline:
                        ...
                        print_need_newline = False
                    print(
                        x.ljust(40), str(w[x].dtype).replace("torch.", "").ljust(10), w[x].device
                    )
                else:
